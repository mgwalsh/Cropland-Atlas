...
The main end product of all of this wrangling is a dataframe called `gsdat` which contains the various GeoSurvey observations as well the gridded observations and corresponding administrative boundaries from [**GADM**](https://gadm.org/data.html). You can look at the structure of this file with:

```{r}
str(gsdat)
```

Note that there are n = 16,219 (6.25 ha) quadrats (rows) and 69 variables (columns) in this dataframe. The actual GeoSurvey variables that are to be predicted from the spatial covariates in this particular case include:
...
BP, CP, and WP predictions are used to generate a Land Cover Classification (LCC) focused on anthropic parts of landscapes, which we use for survey post-stratification, small area estimation and development of ground-sampling plans (e.g, for soil and cropping systems sampling plans that involve sending teams into the field).
...
GeoSurvey contributions are acknowledged as a simple wordcloud, which highlights the proportion of the quadrats that were completed by each GeoSurveyor. This essentially provides a survey signature for quality control purposes.
...
We now turn to the prediction part of the workflow. Everyone appears to have their favorite algorithm(s) for predicting the types geospatial data that GeoSurvey produces. The notion of there being favorite algorithms supports our philosophy that we might be able use some sort of a combination of those to derive a weighted (*"ensemble"*) prediction at the end of the process. The basic schema for this is shown in the Figure below:
