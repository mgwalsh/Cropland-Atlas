...
In order to monitor changing landscapes these steps should be repeated over time; i.e., to facilitate step 4. The end of this chapter gives some suggestions on as to how to do just that, though we cannot provide a fully-worked example of GeoSurvey-based space-time prediction at this point in time. 
...
We will demonstrate the use the following R packages to assemble a coherent dataframe for Tanzania, as an example for the overall workflow:
...
### Load the data to your computer

There are already many examples of GeoSurveys of different countries and other regions of interest that are available. For Africa, exemplar *"tidied-up"* data are made publicly available on the [**Open Science Framework**](https://osf.io/vxc97/) repository, which also mirrors all of the associated data wrangling and prediction R-code at [**Github**](https://github.com/mgwalsh/Cropland-Atlas).

This chunk loads all of the relevant data for Tanzania from the [OSF repo]() & Dropbox:
...
People have asked what are all the covariates that are used in the various predictions? While these keep changing as we get new open access to data and they are often long-term averages which are being updated as warranted. For Tanzania they currently include:

1. Climate related covariates:
  - temperature maps,
  - precipitation maps,
2. Terrain data
3.
...
The following chunk writes and documents files that are that will be used subsequently for various spatial predictions. It also produces simple Leaflet maps of sampling locations and building coordinates and attributes GeoSurveyor contributions as a wordcloud.
