...
3. Central place theory based variables from various sources
    ...
    - distances to national, parks, wildlife conservation areas and forest reserves
...
5. Previous predictions of soil and land cover models e.g.,
    - ESA land cover fraction [**Copernicus**](http://lcviewer.vito.be/)
    ...
    - AfSIS results from previous GeoSurveys [**AfSIS**](https://osf.io/vxc97/)
    
The full current listing of the Tanzania grids is available is available at [**iSDA report 2019**](https://osf.io/7acs3/). All of the layers were resampled to 250 m resolution on a common Lambert-Azimuthal Equal Area coordinate reference system `CRS(+proj=laea +ellps=WGS84 +lon_0=20 +lat_0=5 +units=m +no_defs")`. Note that the production of the grid stack is done outside of R, in [**GRASS**](https://grass.osgeo.org/).
  
### Assemble the data {#geosurvey_step1}

The chunk below assembles the data. It includes total building counts and cropland proportion estimates based on a grid count (both per 6.25 ha quadrat) and subsequently combines everything with the gridded variables in a consistent dataframe that can be used for spatial prediction.
...
The assembly of the dataframe called `gsdat` which now contains the various GeoSurvey observations as well the gridded observations and corresponding administrative boundaries from [**GADM**](https://gadm.org/data.html). You can look at the structure of this file with:

```{r}
str(gsdat)
```

Note that there are 16,219 (6.25 ha) quadrats (rows) and 69 variables (columns) in this dataframe. The actual GeoSurvey variables that are to be predicted from the spatial covariates in this particular case include:
...
5. **bcount**, a tagged count of all observed buildings in a quadrat
6. **ccount**, a grid count of the proportion of cropland area (n/16) in a quadrat

The following chunk writes the `gsdat` file and others that are subsequently used for various spatial predictions. It also produces simple Leaflet maps of sampling locations and/or tagged building coordinates.
...
GeoSurvey contributions are acknowledged as a simple wordcloud, which highlights the proportion of the quadrats that were completed by each GeoSurveyor. This com
...
This completes the data wrangling section (step 1 of the overall workflow).
