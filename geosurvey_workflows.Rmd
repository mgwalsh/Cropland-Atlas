...
The following chunk loads all of the relevant data for Tanzania from its [**OSF repo**](https://osf.io/j8y3z/) & Dropbox:

```{r}
# set working directory
dir.create("Notebooks", showWarnings = F)
setwd("./Notebooks")
dir.create("/figures")
dir.create("/Results")
...
download("httpshttps://osf.io/9gndf?raw=1", "TZ_geos_2018.csv.zip", mode = "wb")
...
We now turn to the prediction part of the workflow. We acknowledge that many data scientists appear to have their favorite algorithm(s) for predicting the types geospatial data that GeoSurvey produces. Alsomany new algorithms are being developed. The notion of there being favorite algorithms supports our philosophy that we might be able use some sort of a combination of those (favorites) to derive a weighted (*"ensemble"*) prediction at the end of the process. Our basic schema for this is shown in the Figure below:

```{r geosurvey_prediction_wokflow, echo=FALSE, fig.cap="Generalized GeoSurvey ensemble prediction workflow", out.width="100%", out.extra="angle=0"}
knitr::include_graphics("figures/geosurvey_prediction_wokflow.png")
```
